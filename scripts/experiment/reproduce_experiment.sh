...
REPEATS="${REPEATS:-3}"
LIMIT="${LIMIT:-5}"
WORKLOAD="${WORKLOAD:-workloads/math_experiment.jsonl}"
...
for repeat in $(seq 1 "${REPEATS}"); do
  ...
  "${PYTHON}" scripts/experiment/experiment.py --condition chatbot --run-id "${run_id}" \
    --workload "${WORKLOAD}" --limit "${LIMIT}"
  "${PYTHON}" scripts/experiment/experiment.py --condition agent --run-id "${run_id}" \
    --workload "${WORKLOAD}" --limit "${LIMIT}"
done
